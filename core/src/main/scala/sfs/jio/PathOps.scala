...
final class PathOps(private val path: Path) extends AnyVal {
  def follow: PathOps.FollowOps     = new PathOps.FollowOps(path)
  def nofollow: PathOps.NoFollowOps = new PathOps.NoFollowOps(path)
...
object PathOps {
  final class FollowOps(private val path: Path) extends AnyVal {
  ...
  final class NoFollowOps(private val path: Path) extends AnyVal {
